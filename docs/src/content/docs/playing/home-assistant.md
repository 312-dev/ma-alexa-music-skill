...
description: Driving Music Assistant from Home Assistant, including multi-room and the duplicate-entity trap.
---
...
data:
  media_content_type: custom
  media_content_id: >-
    ask music assistant to play {{ artist }} on {{ target_group }}
```
...
- **You cannot make Music Assistant the default provider**, so the alias is always in the
  utterance.
