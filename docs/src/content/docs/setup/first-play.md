...
Stand near an Echo. Substitute your own alias for `ampere`.
...
| Say | You get |
|---|---|
| `play <artist> on ampere` | That artist's whole discography, shuffled |
| `play <album> on ampere` | The album, in album order |
| `play <track> on ampere` | That track |
| `play the <name> playlist on ampere` | The playlist, in playlist order |
| `play <genre> on ampere` | That genre, shuffled |
| `play <artist> radio on ampere` | A [station](../../how-it-works/stations/) built from that artist and artists like them |
| `play <artist> station on ampere` | The same |
| `play ampere` | Your library at random |

Albums and playlists keep their own order, because their order is the point.
Artists, genres, starred tracks, the random library and stations shuffle by
default: asking for an artist and getting the first track of their earliest
album, every time, is not what anyone means.
...
curl -s -H "X-Admin-Token: $ADMIN_TOKEN" \
  https://ampere.example.com/captures | head -c 2000
```

The container log gives the same shape with timings:

```sh
docker logs --tail 50 ampere
```
