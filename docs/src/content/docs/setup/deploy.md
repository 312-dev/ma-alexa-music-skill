...
description: Building and running Music Assistant with Docker, the environment variables it reads, and the data it needs on disk.
sidebar:
  order: 5
---
...
```sh
docker pull ghcr.io/graysoncadams/ma-alexa-music-skill:latest
```
...
```sh
docker pull ghcr.io/graysoncadams/ma-alexa-music-skill@sha256:...
```
...
```sh
git clone https://github.com/GraysonCAdams/ma-alexa-music-skill.git
cd ma-alexa-music-skill
docker build -t ma_alexa .
```
...
```sh
cp .env.example .env
docker volume create ma-alexa-data

docker run -d --name ma_alexa --restart unless-stopped \
  -p 127.0.0.1:5056:5056 \
  --env-file .env \
  -v ma-alexa-data:/data \
  ghcr.io/graysoncadams/ma-alexa-music-skill:latest
```
...
services:
  ma_alexa:
    image: ghcr.io/graysoncadams/ma-alexa-music-skill:latest
