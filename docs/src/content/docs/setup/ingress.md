...
labels:
  - "traefik.enable=true"
  - "traefik.http.routers.ma_alexa.rule=Host(`music.example.com`)"
  - "traefik.http.routers.ma_alexa.entrypoints=websecure"
  - "traefik.http.routers.ma_alexa.tls.certresolver=letsencrypt"
  - "traefik.http.services.ma_alexa.loadbalancer.server.port=5056"
```
...
Music Assistant does not automate tunnel configuration and will not ask for a Cloudflare
