...
curl -s -H "X-Admin-Token: $ADMIN_TOKEN" https://music.example.com/captures

# 2. Is the skill actually reaching playback?
curl -s -H "X-Admin-Token: $ADMIN_TOKEN" https://music.example.com/setup/skill/health

# 3. Can the bridge reach the music server?
curl -s -H "X-Admin-Token: $ADMIN_TOKEN" 'https://music.example.com/diag?q=radiohead'

# 4. What did each handler do, and how long did it take?
docker logs --tail 100 ma_alexa
```
...
curl -s -H "X-Admin-Token: $ADMIN_TOKEN" \
  https://music.example.com/setup/skill/health
```
...
```sh
curl -sI https://music.example.com/icons/ma-512.png
```
...
at `Initiate` for any queue that will later continue into one.
