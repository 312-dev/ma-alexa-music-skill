...
def test_the_image_copies_the_package_whole_rather_than_module_by_module():
    ...
    Music Assistant loads `providers/ma_alexa` and a provider may only import
...
def test_importing_the_core_never_kills_the_host_process():
    ...
    That was defensible while Music Assistant was a process of its own: refusing to
    start beats serving stream URLs Amazon cannot fetch. Inside Music Assistant
    the same import happens during MA's startup, so an unset Music Assistant setting
...
def test_importing_the_package_has_no_side_effects():
    ...
    Music Assistant was written as an application, and an application may do things at
    ...
    would have reformatted MA's logs and teed them into Music Assistant's ring buffer,
    and did no harm only because MA configures logging first, which is luck.
    `mkdir` put Music Assistant's directories in MA's storage root beside library.db.
...
def test_every_injected_secret_reaches_the_module_that_uses_it():
    ...
    try:
        app_module.configure(
            ...
            oauth_client_id="ma-alexa-abcd1234",
        ...
        assert oauth.CLIENT_ID == "ma-alexa-abcd1234"
