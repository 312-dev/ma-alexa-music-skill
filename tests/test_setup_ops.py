...
def isolated(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(core, "PUBLIC_BASE", "https://ma_alexa.example.test")
...
def passing_endpoint():
    store.update(endpoint_ok=True, alias="ma_alexa", cert_type="Trusted")
...
def test_a_skill_is_created_once_the_endpoint_passes(monkeypatch):
    ...
    outcome = setup_ops.create_skill(alias="ma_alexa",
...
def test_creation_refuses_without_a_passing_endpoint():
    ...
    """
    store.update(endpoint_ok=False, alias="ma_alexa")
    outcome = setup_ops.create_skill(alias="ma_alexa",
                                     public_base="https://ma_alexa.example.test")
...
def test_creation_refuses_a_plain_http_base():
    ...
    passing_endpoint()
    outcome = setup_ops.create_skill(alias="ma_alexa",
                                     public_base="http://ma_alexa.example.test")
    assert outcome.ok is False
...
def test_the_manifest_carries_the_derived_certificate_type(monkeypatch):
    ...
    setup_ops.create_skill(alias="ma_alexa",
                           public_base="https://ma_alexa.example.test")
...
def test_the_vendor_is_passed_through(monkeypatch):
    ...
    setup_ops.create_skill(alias="ma_alexa", vendor="M2VENDOR",
                           public_base="https://ma_alexa.example.test")
...
def test_an_orphan_is_reused_rather_than_duplicated(monkeypatch):
    ...
    monkeypatch.setattr(smapi_rest, "list_catalogs",
                        lambda: [{"title": "Music Assistant artists", "id": "orphan-1"}])
    monkeypatch.setattr(smapi_rest, "create_catalog",
...
# --- the one-button orchestrator --------------------------------------------


def test_provision_creates_the_skill_then_the_catalogs(monkeypatch):
    """The whole of the one-button setup, minus the upload the caller hands
    off. Skill first because catalogs cannot associate without one."""
    passing_endpoint()
    order = []
    monkeypatch.setattr(smapi_rest, "create_skill",
                        lambda manifest, vendor: order.append("skill")
                        or "amzn1.ask.skill.new")
    monkeypatch.setattr(smapi_rest, "list_catalogs",
                        lambda: order.append("list") or [])
    monkeypatch.setattr(smapi_rest, "create_catalog",
                        lambda title, kind: f"cat-{kind}")
    monkeypatch.setattr(smapi_rest, "associate_catalog",
                        lambda skill, catalog: None)

    outcome = setup_ops.provision(alias="ma_alexa",
                                  public_base="https://ma_alexa.example.test")
    assert outcome.ok
    assert order[0] == "skill" and "list" in order
    assert store.load()["skill_id"] == "amzn1.ask.skill.new"
    assert set(store.load()["catalogs"]) == set(setup_ops.CATALOG_KINDS)


def test_provision_skips_catalogs_that_already_exist(monkeypatch):
    """An ongoing Sync must not spend five SMAPI calls re-listing catalogs it
    already has. list_catalogs is forbidden here, so a wrongful call is a
    loud failure rather than a silent inefficiency."""
    passing_endpoint()
    store.update(skill_id="amzn1.ask.skill.x",
                 catalogs={k: f"cat-{k}" for k in setup_ops.CATALOG_KINDS})

    outcome = setup_ops.provision(alias="ma_alexa",
                                  public_base="https://ma_alexa.example.test")
    assert outcome.ok
    assert any(r["kind"] == "catalogs" and "reused" in r["detail"]
               for r in outcome.rows)


def test_provision_stops_at_the_skill_when_it_fails():
    """A failed skill step must not fall through to the catalogs. With the
    endpoint unproven, create_skill refuses; create_catalogs would then reach
    list_catalogs, which is forbidden, so a fall-through fails loudly."""
    store.update(endpoint_ok=False, alias="ma_alexa")
    outcome = setup_ops.provision(alias="ma_alexa",
                                  public_base="https://ma_alexa.example.test")
    assert outcome.ok is False
...
def test_connecting_refuses_without_an_https_public_base(monkeypatch):
    ...
    monkeypatch.setattr(core, "PUBLIC_BASE", "http://ma_alexa.example.test")
...
def test_the_ma_path_refuses_an_empty_provider_selection(monkeypatch):
    """With a live Music Assistant and no providers chosen, the upload must not
    fall back to crawling Subsonic: it refuses and says what to pick."""
    from ma_provider import catalog_sync
    ...
    assert outcome.ok is False
    assert "provider" in outcome.detail.lower()
    assert crawled == [], "an empty selection must not crawl Subsonic"


def test_the_ma_path_reads_from_the_chosen_providers(monkeypatch):
    """A non-empty selection sources the catalog from MA's library, not the
    Subsonic crawl."""
    from ma_provider import catalog_sync
    ...
    monkeypatch.setattr(catalog_sync, "collect_from_ma", fake_from_ma)
    monkeypatch.setattr(catalog_sync, "apply_timestamps",
                        lambda kind, entities, saved: (entities, "new"))
    monkeypatch.setattr(smapi_rest, "upload_catalog",
                        lambda catalog_id, payload: "up-9")
    ...
    outcome = setup_ops.run_upload(mass=object(), loop=object())
    assert outcome.ok
    assert seen.get("providers") == ["library--navidrome"]
    assert "collect" not in seen, "the MA path must not call the Subsonic crawl"
