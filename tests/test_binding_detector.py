...
from ma_provider import binding
...
SEARCH = binding.SEARCH_DIRECTIVE
INITIATE = binding.INITIATE_DIRECTIVE
PLAYING = binding.PLAYING_DIRECTIVE
...
def test_a_search_answered_by_initiate_counts_as_reaching_playback(isolated):
    ...
    health = binding._binding_health()
...
def test_the_capture_index_survives_a_scrub_too(isolated):
    """Same defect, same fix: the index must stay ordered.

    "Did anything arrive recently" is what the endpoint step and the keep-alive
    both ask, so reporting a rewritten capture as having just arrived is the
    one wrong answer it must not give.
    """
    scrub = time.time()
    write_capture(isolated, 9000, SEARCH, mtime=scrub)
    write_capture(isolated, 30, INITIATE, mtime=scrub)

    names = [name for name, _ in captures.recent(4)]
...
def test_recent_captures_keeps_a_foreign_filename(isolated):
    ...
    assert [n for n, _ in captures.recent(4)] == ["stray.json"]
...
def test_capture_time_reads_the_name_not_the_clock(isolated):
    ...
    assert captures.capture_time("20260802T153412987654-x.json") == pytest.approx(
        datetime(2026, 8, 2, 15, 34, 12, 987654, tzinfo=timezone.utc).timestamp())
    assert captures.capture_time("not-a-capture.json") is None
    assert captures.capture_time("20261302T999999000000-x.json") is None
...
def armed_skill(monkeypatch):
    ...
    cycles = []
    monkeypatch.setattr(
        setup_ops, "cycle_enablement",
        lambda skill_id: (cycles.append(skill_id),
                          setup_ops.Outcome(True, "Re-provisioned."))[1])
    return cycles
...
def test_a_run_of_misses_triggers_exactly_one_cycle(isolated, armed_skill):
    ...
    write_capture(isolated, 300, SEARCH)
    log = FakeLog()
    binding._reactive_check(store.load(), time.time(), log)
...
def test_a_miss_that_playback_followed_is_not_part_of_a_run(isolated, armed_skill):
    ...
    write_capture(isolated, 300, SEARCH)
    binding._reactive_check(store.load(), time.time(), FakeLog())
...
def test_it_does_nothing_without_a_skill(isolated, monkeypatch):
    ...
    monkeypatch.setattr(smapi_rest, "connected", lambda: True)
    cycles = []
    monkeypatch.setattr(setup_ops, "cycle_enablement",
                        lambda sid: cycles.append(sid) or
                        setup_ops.Outcome(True, "cycled"))
    write_capture(isolated, 400, SEARCH)
    write_capture(isolated, 300, SEARCH)
    binding._reactive_check(store.load(), time.time(), FakeLog())
    assert cycles == []
...
def test_a_zero_interval_turns_the_keepalive_off(isolated, monkeypatch):
    ...
    assert binding._binding_stale(fresh, time.time()) is True
    monkeypatch.setattr(binding, "BINDING_KEEPALIVE_HOURS", 0)
    assert binding._binding_stale(fresh, time.time()) is False
