...
Three things make a live suite against Ampere different from an ordinary
...
**State is optimistic.** Ampere writes the state it expects and calls
`update_state()` before Alexa has done anything, then re-polls 1.5s later. A
read taken immediately after a command confirms Ampere's optimism, not the
...
ENVIRONMENT: dict[str, Any] = json.loads((HERE / "environment.json").read_text())

# --- what Ampere's own constants say a test is allowed to expect -------------
#
...
RESYNC_SECONDS = 1.5          # Ampere re-polls Alexa this long after a control
...
DEFAULT_WS_URL = os.environ.get(
    "MA_WS_URL", ENVIRONMENT["extra"]["ma_tailnet_url"].replace("http://", "ws://") + "/ws"
)
BOX = os.environ.get("AMPERE_BOX", "hetzner")
...
class Observation:
    ...
    - `event_ms` when MA published a changed snapshot. For an Ampere control
...
def _read_token() -> str:
    ...
    result = subprocess.run(
        ["ssh", "-o", "BatchMode=yes", BOX, "cat /opt/ampere/.ma-token"],
    ...
    if result.returncode != 0 or not token:
        raise RuntimeError(
            ...
            f"so that {BOX}:/opt/ampere/.ma-token exists"
        )
    return token
...
class LiveSession:
    ...
    def player_elapsed(self, player_id: str) -> tuple[float, float]:
        ...
        `player.elapsed_time` is Ampere's record of what it last read off Alexa,
...
def observe(
    ...
    *,
    ...
    The floor is not a convenience sleep. Ampere answers controls optimistically:
...
class Ampere:
    ...
    def discover(self) -> dict[str, Target]:
        ...
        preferred = os.environ.get("AMPERE_LIVE_SPEAKER", "Bedroom Echo")

        singles: list[Target] = []
        group: Target | None = None
        for player_id, player in players.items():
            if not str(player.get("provider", "")).startswith("ampere"):
                continue
...
def _markdown(payload: dict[str, Any]) -> str:
    lines: list[str] = [
        f"# Ampere live conformance - {payload['generated']}",
        "",
    ...
    lines += ["", "## Latency", "",
              ...
              "published state change (optimistic for Ampere controls), `effect` "
