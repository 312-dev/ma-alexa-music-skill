...
Ampere is a Music Assistant player provider whose speakers are Amazon Echos on
...
Ampere is wrong has done its job. See "Reading the output" for how to tell that
...
- `ssh hetzner` works. The Music Assistant API token is read from
  `/opt/ampere/.ma-token` on the box, into memory, and is never printed, logged
...
| `AMPERE_BOX` | `hetzner` | ssh host holding the token |
| `AMPERE_LIVE_SPEAKER` | `Bedroom Echo` | which cleared speaker is the single-player target |

`AMPERE_LIVE_SPEAKER` is a preference, not a permission: an unrecognised name
...
**Nothing looks too early.** Ampere writes the state it expects and publishes it
*before* Alexa has been asked anything, then re-polls 1.5s later. A read taken
immediately confirms Ampere's optimism, not the speaker. `harness.observe()`
...
for 18 reports 17 - so volume is asserted to +/-2, the same tolerance Ampere's
...
- **Latency table** - p50 and max per action type across three clocks:
  ...
  | `event` | MA publishing a changed snapshot. For an Ampere control this is the *optimistic* write - it is a measure of the server, not of the speaker. |
...
A failing assertion is a claim about Ampere, and the message carries the
...
cases deliberately assert on `player.elapsed_time` (what Ampere read back off
...
id, which changes when the provider is re-added, so `Ampere.discover()` matches
