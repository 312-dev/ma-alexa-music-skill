...
def test_a_cleared_speaker_is_allowed():
    bedroom = "ampere--3wyAVB8M:G2A1A603050507A4"
...
def test_every_excluded_device_is_refused(serial):
    with pytest.raises(safety.Unsafe):
        safety.check(f"ampere--3wyAVB8M:{serial}")
...
def test_an_unknown_device_is_refused_rather_than_allowed():
    ...
    with pytest.raises(safety.Unsafe):
        safety.check("ampere--3wyAVB8M:BRAND_NEW_DEVICE")
...
def test_refusal_raises_rather_than_returning_false():
    ...
    with pytest.raises(safety.Unsafe) as caught:
        safety.check("ampere--3wyAVB8M:eeb2eea6b4174cf19db1acab6c59bb62")
...
def test_the_two_living_room_devices_are_told_apart():
    ...
    """
    studio = "ampere--3wyAVB8M:G2A0XL12229302V4"
    plain = "ampere--3wyAVB8M:G6G22J0633840B05"
...
def test_a_group_is_only_as_safe_as_its_worst_member():
    ...
    ok, blockers = safety.group_is_safe([
        "ampere--3wyAVB8M:G2A1A603050507A4",
        "ampere--3wyAVB8M:eeb2eea6b4174cf19db1acab6c59bb62",
    ])

    assert ok is False
    assert blockers == ["ampere--3wyAVB8M:eeb2eea6b4174cf19db1acab6c59bb62"]


def test_a_group_of_cleared_speakers_is_safe():
    ok, blockers = safety.group_is_safe([
        "ampere--3wyAVB8M:G2A1A603050507A4",
        "ampere--3wyAVB8M:G6G22M0625320M40",
    ])
...
def test_a_bare_serial_works_as_well_as_a_player_id():
    ...
    assert safety.serial_of("ampere--x:G2A1A603050507A4") == "G2A1A603050507A4"
