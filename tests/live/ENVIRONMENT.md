...
suite runs against: the container it lives in, the Music Assistant provider instance, the
...
- `ssh my-box` works (BatchMode is fine).
...
```bash
ssh my-box 'docker logs --since 10m app-<alloc> 2>&1 | tail -20'
```
...
```bash
ssh my-box 'nomad job allocs -json music-assistant | python3 -c "import json,sys; print([x[\"ID\"] for x in json.load(sys.stdin) if x[\"ClientStatus\"]==\"running\"])"'
```
...
- **The allow/exclude split is a user decision, not a property of the system.**
  Only Living Room, Bathroom Echo, Kitchen Echo and Living Room Echo Studio may
...
At the time of writing, the Music Assistant provider exposed exactly one Alexa speaker
...
- `/opt/music-assistant/data/ma_alexa/mastream/` holds Music Assistant's cached streams.
...
| proven-streamed URIs | `/opt/music-assistant/data/ma_alexa/mastream/` filenames (base64url) |
...
```bash
ssh my-box 'mkdir -p /tmp/malib && cp /opt/music-assistant/data/library.db /opt/music-assistant/data/library.db-wal /tmp/malib/'
```
