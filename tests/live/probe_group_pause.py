...
the matching Ampere log lines dumped afterwards.
...
from tests.live.harness import Ampere, LiveSession
...
def main() -> int:
    ...
    session.start()
    try:
        ampere = Ampere(session)
        targets = ampere.discover()
        group = targets["group"]
        ...
        _, played = ampere.arrange_playing(group, source, tracks=1)
        print(f"\nplay: ok={played.ok} {played.detail}")
        settled = ampere.settle(group)
