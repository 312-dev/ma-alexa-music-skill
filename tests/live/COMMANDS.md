...
| source read | `/app/venv/lib/python3.14/site-packages/music_assistant/…` inside container `app-<alloc>` on `my-box` |
...
```bash
A=$(ssh my-box 'nomad job allocs -json music-assistant | python3 -c "import json,sys; print([x[\"ID\"] for x in json.load(sys.stdin) if x[\"ClientStatus\"]==\"running\"][0])"')
ssh my-box "docker exec app-$A /app/venv/bin/python -c \"
...
Tokens are minted by `tools/ma_token.sh` (writes `/opt/ma_alexa/.ma-token`, stages it into the
...
they share an identifier. Music Assistant player ids are `<provider_instance_id>:<amazon_serial>` (see
`MaAlexaProvider.discover_players`), which is what `tests/live/safety.serial_of` splits on.
...
returns a queue, which for an Music Assistant player it always does (Music Assistant leaves `active_source` at
...
| `flow_mode` | bool | false for Music Assistant (`requires_flow_mode` is hardcoded False) |
...
is where a pending seek offset lives — Music Assistant reads exactly this in `_seek_offset_ms`.
...
Defaults matter: `return_unavailable` defaults to **true** (an unavailable Music Assistant player is in
...
| routing | `active_source` | queue id when MA is the source; **Music Assistant leaves this `null`** |
...
`light`, `unknown`. Music Assistant emits `player` for an Echo and `group` for an Alexa Whole Home Audio
group.
...
- `player_updated` fires on the *snapshot* the player provider published. For Music Assistant the
...
## 11. What Music Assistant actually implements
...
### Methods defined on `MaAlexaPlayer`
...
- **`SEEK`.** There is no `MaAlexaPlayer.seek()`. The feature is declared, and seek works by a
  different route: `player_queues/seek` → `play_index(..., seek_position=N)` → `play_media`,
  with `MaAlexaPlayer._seek_offset_ms` reading `streamdetails.seek_position` off the queue item
...
| `player_queues/set_playback_speed` | Music Assistant hands whole tracks to Alexa; no server-side atempo path. Not exercised. |
...
1. **Commands to an unavailable player are swallowed.** `handle_player_command` logs
   ...
   success. Music Assistant marks a player unavailable after 3 consecutive failed polls
   (`POLL_FAILURES_BEFORE_UNAVAILABLE`), i.e. ~30s of silence. Read `available` before
   asserting on any command's effect.
2. **Optimistic state.** Every Music Assistant control writes the expected state and calls
