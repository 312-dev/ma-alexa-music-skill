...
# --- library and favorites voice intents ------------------------------------


@pytest.mark.parametrize("spoken", [
    "my library", "my music", "all my music", "everything",
    "shuffle", "shuffle my music", "Play My Library", "  my   music  ",
])
def test_library_intent_maps_whole_library_phrases(app, spoken):
    assert app._library_intent(spoken) == "rnd:all"


@pytest.mark.parametrize("spoken", [
    "my favorites", "favorites", "starred", "liked songs", "songs i like",
    "my favourites", "songs i like.", "Play My Favorites",
])
def test_library_intent_maps_favorites_phrases(app, spoken):
    assert app._library_intent(spoken) == "star"


@pytest.mark.parametrize("spoken", [
    "Gregory Alan Isakov", "Appaloosa Bones", "bedtime playlist",
    "everything but the girl", "", "jazz",
])
def test_library_intent_leaves_real_titles_alone(app, spoken):
    assert app._library_intent(spoken) is None


def test_library_phrase_plays_the_random_library(client):
    """'shuffle my music' reaches the library, not a track titled closest."""
    out = post(client, directive(
        "Alexa.Media.Search", "GetPlayableContent",
        {"selectionCriteria": {"attributes": [{"value": "shuffle my music"}]}},
    ))
    content = out["payload"]["content"]
    assert content["id"] == "rnd:all"
    assert content["metadata"]["name"]["speech"]["text"] == "My Library"


def test_favorites_phrase_plays_the_starred_songs(client):
    out = post(client, directive(
        "Alexa.Media.Search", "GetPlayableContent",
        {"selectionCriteria": {"attributes": [{"value": "my favorites"}]}},
    ))
    content = out["payload"]["content"]
    assert content["id"] == "star"
    assert content["metadata"]["name"]["speech"]["text"] == "My Favorites"


def test_a_real_artist_is_not_hijacked_by_the_library_intent(client):
    """A named artist still resolves to that artist, never to the library."""
    out = post(client, directive(
        "Alexa.Media.Search", "GetPlayableContent",
        {"selectionCriteria": {"attributes": [
