...
from ma_provider import tasks as ampere_tasks
...
def adapter(tmp_path, monkeypatch):
    ...
    return ampere_tasks.AmpereTasks(FakeMass(), logging.getLogger("test"))
...
def test_an_upload_is_queued_under_a_fixed_id(adapter):
    ...
    assert ids == [ampere_tasks.UPLOAD_TASK_ID] * 2
...
def test_an_interval_below_the_floor_is_raised_to_it(adapter):
    ...
    assert schedule.every == ampere_tasks.MIN_SYNC_HOURS
...
async def test_progress_from_the_worker_thread_is_marshalled_to_the_loop(
    ...
    def fake_run_upload(*, progress, should_stop, cycle_after):
