...
def test_the_modifier_is_ordered_after_autofill():
    ...
    assert keys.index("autofill") < keys.index("ma_alexa_clear_register_form")
...
def test_every_page_amazons_login_can_visit_is_routed():
    ...
    for method, path in (
        ("GET", "/ma_alexa/auth/proxy/"),
        ("GET", "/ma_alexa/auth/proxy/ap/signin"),
        ("POST", "/ma_alexa/auth/proxy/ap/signin/146-931"),
        ("GET", "/ma_alexa/auth/proxy/ap/cvf/verify"),
        ("POST", "/ma_alexa/auth/proxy/ap/cvf/verify"),
        ("GET", "/ma_alexa/auth/proxy/ap/mfa"),
        ("POST", "/ma_alexa/auth/proxy/errors/validateCaptcha"),
    ):
...
def test_the_old_two_route_registration_would_have_missed_it():
    ...
    old = {
        "GET./ma_alexa/auth/proxy/": object(),
        "POST./ma_alexa/auth/proxy/ap/signin/*": object(),
    }

    assert not _ma_catch_all_matches(old, "POST", "/ma_alexa/auth/proxy/ap/cvf/verify")
...
def test_the_wildcard_does_not_swallow_unrelated_paths():
    ...
    assert not _ma_catch_all_matches(registered, "GET", "/ma_alexa/other")
