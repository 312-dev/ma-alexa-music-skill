...
def test_handoff_phrase_matching():
    assert queue_api.is_handoff_phrase("my mix")
    assert queue_api.is_handoff_phrase("My Mix")
    assert queue_api.is_handoff_phrase("  my   mix  ")
    # Speech routinely appends the noun, and app.py strips the same words.
    assert queue_api.is_handoff_phrase("my mix playlist")
    assert queue_api.is_handoff_phrase("My Mix, Playlist")
    assert not queue_api.is_handoff_phrase("mix")
    assert not queue_api.is_handoff_phrase("mix my")
...
def subsonic_track(sid="t1", **extra):
    """A Subsonic track the provider already resolved from MA's metadata."""
    track = {
        "source": "subsonic",
        "id": sid,
        "title": "Pre Song",
...
def test_a_prepared_subsonic_track_publishes_without_a_lookup(queue_client, monkeypatch):
    """The whole point: MA had the metadata, so the bridge must not getSong it.

    This is what lets a 30k-track queue publish without a round trip per track.
    """
    calls = []
    ...
    resp = publish(queue_client, [subsonic_track("t1")])
    assert resp.status_code == 200
    token = resp.get_json()["content_id"].split(":", 1)[1]

    (song,) = queue_api.resolve(token)
    assert calls == [], "a pre-resolved Subsonic track must not be fetched"
    assert song["id"] == "t1"
    assert not song.get("ma_ref"), "it streams straight from Subsonic, not via MA"
    assert song["title"] == "Pre Song"
    assert song["artist"] == "Pre Artist"
    assert song["duration"] == 200


def test_a_prepared_subsonic_track_hashes_like_its_bare_id(queue_client):
    """So re-publishing the same queue lands on the same record and the stale
    seek self-clears, whichever shape the provider happened to send."""
    bare = publish(queue_client, ["t1", "t2"]).get_json()["content_id"]
    prepared = publish(
        queue_client, [subsonic_track("t1"), subsonic_track("t2")]
    ).get_json()["content_id"]
    assert bare == prepared


def test_a_prepared_subsonic_track_without_a_title_is_dropped(queue_client):
    """Validated like a fetched record: no title, no track."""
    resp = publish(queue_client, [subsonic_track("t1"),
                                  subsonic_track("t2", title="")])
    token = resp.get_json()["content_id"].split(":", 1)[1]
    assert [s["id"] for s in queue_api.resolve(token)] == ["t1"]
...
# --- appending to a live queue (the growing-token spike) --------------------
#
# The question this prototype exists to gate: can a published `ext:` queue grow
# after publish and have Alexa keep pulling the new tracks off the same
# contentId, with no re-Initiate? These tests establish the two things the code
# side needs -- resolve() sees the append, and core's resolve cache no longer
# masks it. Whether Alexa actually keeps *asking* once it is near the end is an
# Alexa-side behaviour only tools/growing_token_probe.py can settle.


def test_appending_extends_what_resolve_returns(queue_client):
    """The base claim: the grown queue is what resolve() hands back."""
    token = publish(queue_client, ["t1", "t2"]).get_json()["content_id"].split(":", 1)[1]
    assert [s["id"] for s in queue_api.resolve(token)] == ["t1", "t2"]

    updated = queue_api.append_to_queue(token, ["t3"])
    assert updated is not None
    assert [s["id"] for s in queue_api.resolve(token)] == ["t1", "t2", "t3"]


def test_appending_keeps_the_original_token(queue_client):
    """Alexa is holding ext:<token> and echoes it back; the append must not
    re-hash the list and orphan it."""
    content_id = publish(queue_client, ["t1", "t2"]).get_json()["content_id"]
    token = content_id.split(":", 1)[1]
    updated = queue_api.append_to_queue(token, ["t3"])
    assert updated["token"] == token


def test_appending_accepts_the_same_shapes_publish_does(queue_client):
    """A bare Subsonic id and a source==ma dict both append, in order."""
    token = publish(queue_client, ["t1"]).get_json()["content_id"].split(":", 1)[1]
    queue_api.append_to_queue(token, [ma_track("spotify://track/z"), "t2"])
    songs = queue_api.resolve(token)
    assert [s["id"] for s in songs] == [
        "t1", f"{queue_api.MA_ID_PREFIX}{ma_track('spotify://track/z')['ref']}", "t2",
    ]
    assert [bool(s.get("ma_ref")) for s in songs] == [False, True, False]


def test_appending_to_an_unknown_token_returns_none(queue_client):
    assert queue_api.append_to_queue("deadbeef", ["t1"]) is None


def test_appending_to_an_expired_token_returns_none(queue_client, monkeypatch):
    """An append onto a queue Alexa has already dropped is nothing to grow."""
    token = publish(queue_client, ["t1"]).get_json()["content_id"].split(":", 1)[1]
    monkeypatch.setattr(queue_api, "_expired", lambda record, now=None: True)
    assert queue_api.append_to_queue(token, ["t2"]) is None


def test_appending_nothing_valid_leaves_the_queue_unchanged(queue_client):
    """A dropped-on-validation append is a no-op, not a failure."""
    token = publish(queue_client, ["t1", "t2"]).get_json()["content_id"].split(":", 1)[1]
    updated = queue_api.append_to_queue(token, [ma_track(title="  ")])
    assert updated is not None
    assert [s["id"] for s in queue_api.resolve(token)] == ["t1", "t2"]


def test_the_cache_no_longer_masks_the_appended_tracks(queue_client):
    """The crux. resolve_tracks caches the list at the first GetNextItem and
    never re-derives it; without invalidation the append is written to disk and
    never served. This proves append_to_queue clears that cache."""
    token = publish(queue_client, ["t1", "t2"]).get_json()["content_id"].split(":", 1)[1]
    content_id = f"ext:{token}"

    # Prime the cache exactly as the first GetNextItem would.
    assert [s["id"] for s in app_module.resolve_tracks(content_id)] == ["t1", "t2"]
    assert content_id in app_module._QUEUE_CACHE

    queue_api.append_to_queue(token, ["t3"])

    # Invalidated, and the next resolve sees the grown queue.
    assert content_id not in app_module._QUEUE_CACHE
    assert [s["id"] for s in app_module.resolve_tracks(content_id)] == ["t1", "t2", "t3"]


def test_without_invalidation_the_cache_would_mask_the_append(queue_client, monkeypatch):
    """The negative control: neuter the invalidation and the stale cached list
    is what resolve_tracks keeps serving. This is the failure the invalidation
    exists to prevent, made visible."""
    monkeypatch.setattr(queue_api, "_forget_resolved", lambda token: None)
    token = publish(queue_client, ["t1", "t2"]).get_json()["content_id"].split(":", 1)[1]
    content_id = f"ext:{token}"
    ...
    # resolve() (which reads disk) sees the append; resolve_tracks (cached) does
    # not. That gap is exactly what the real _forget_resolved closes.
    assert [s["id"] for s in queue_api.resolve(token)] == ["t1", "t2", "t3"]
    assert [s["id"] for s in app_module.resolve_tracks(content_id)] == ["t1", "t2"]


def test_a_grown_queue_reports_its_requested_count(queue_client):
    """`requested` is what was asked for across publish + every append, so an
    operator can see a queue growing even when a track drops (here "nope" is a
    real id the library does not hold, counted but not served -- same as in
    publish's test_missing_song_does_not_sink_the_queue)."""
    token = publish(queue_client, ["t1", "t2"]).get_json()["content_id"].split(":", 1)[1]
    queue_api.append_to_queue(token, ["t3", "nope"])  # one served, one counted-only
    record = queue_api.append_to_queue(token, [])  # a no-op append, to read it back
    assert record["requested"] == 4  # 2 published + 2 requested by append
    assert [s["id"] for s in record["tracks"]] == ["t1", "t2", "t3"]


# --- the in-process append endpoint (the growing-token route) ---------------


def test_append_endpoint_extends_a_published_queue(queue_client):
    token = publish(queue_client, ["t1", "t2"]).get_json()[
        "content_id"].split(":", 1)[1]
    resp = queue_client.post(f"/queue/{token}/append",
                             json={"tracks": ["t3"]}, headers=AUTH)
    assert resp.status_code == 200
    assert resp.get_json()["count"] == 3
    assert [s["id"] for s in queue_api.resolve(token)] == ["t1", "t2", "t3"]


def test_append_endpoint_runs_in_process_so_the_cache_is_cleared(queue_client):
    """The reason the endpoint exists: resolve_tracks caches at first resolve,
    and only an in-process append clears that cache. Same process here."""
    token = publish(queue_client, ["t1", "t2"]).get_json()[
        "content_id"].split(":", 1)[1]
    content_id = f"ext:{token}"
    assert [s["id"] for s in app_module.resolve_tracks(content_id)] == ["t1", "t2"]
    queue_client.post(f"/queue/{token}/append", json={"tracks": ["t3"]}, headers=AUTH)
    assert [s["id"] for s in app_module.resolve_tracks(content_id)] == ["t1", "t2", "t3"]


def test_append_endpoint_unknown_token_is_404(queue_client):
    resp = queue_client.post("/queue/deadbeef/append",
                             json={"tracks": ["t1"]}, headers=AUTH)
    assert resp.status_code == 404


def test_append_endpoint_requires_auth(queue_client):
    token = publish(queue_client, ["t1"]).get_json()["content_id"].split(":", 1)[1]
    assert queue_client.post(
        f"/queue/{token}/append", json={"tracks": ["t2"]}).status_code == 401


def test_append_endpoint_rejects_a_bad_body(queue_client):
    token = publish(queue_client, ["t1"]).get_json()["content_id"].split(":", 1)[1]
    assert queue_client.post(
        f"/queue/{token}/append", json={"tracks": []}, headers=AUTH).status_code == 400
