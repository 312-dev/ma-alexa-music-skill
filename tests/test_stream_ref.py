...
def test_the_path_a_reference_is_served_at():
    ref = stream_ref.encode_ref(SPOTIFY)
    assert stream_ref.stream_path(ref) == f"/ampere_stream/{ref}.mp3"
...
def test_the_reference_comes_back_out_of_its_path():
    ...
    assert stream_ref.ref_from_path(f"/ampere_stream/{ref}") == ref
    assert stream_ref.ref_from_path(f"/ampere_stream/{ref}.mp3?x=1") == ref
