"""Music Assistant's public endpoint, served from inside Music Assistant.
...
particular endpoint, for a reason that is about MA rather than about Music Assistant.
...
Music Assistant's endpoint has the opposite requirement. Amazon calls it from the public
...
So Music Assistant listens on its own port with its own routing table, and the only
...
The pool is Music Assistant's own rather than `asyncio.to_thread`'s default, for the
...
nobody else is waiting on. Sharing the default executor would let Music Assistant's bad
day become MA's.
"""
...
class MaAlexaWebServer:
    ...
    async def start(self) -> bool:
        ...
        if missing := core.missing_settings():
            self.logger.warning(
                "Music Assistant is not serving the Alexa endpoint yet: %s still to be "
        ...
        self._pool = ThreadPoolExecutor(
            max_workers=POOL_SIZE, thread_name_prefix="ma-alexa-web"
        )
        app = web.Application()
        ...
        try:
            ...
            self.logger.error(
                "Music Assistant could not bind port %s (%s). Something else is already "
                "listening, most likely a standalone Music Assistant deployment. Stop it, "
    ...
    async def stop(self) -> None:
        ...
        if self._pool is not None:
            ...
            self._pool = None
        self.logger.info("Music Assistant endpoint stopped")
    ...
    def _routes(self) -> list[web.RouteDef]:
        return [
            ...
            web.get("/maart/{ident}/{expires}/{sig}", self.maart),
            ...
            web.post("/queue/{token}/append", self.append_queue),
    ...
    async def maart(self, request: web.Request) -> web.StreamResponse:
        return await self._signed(request, core.serve_maart)
    ...
    async def append_queue(self, request: web.Request) -> web.StreamResponse:
        # In-process on purpose: the append's cache invalidation only reaches
        # the worker that runs it, and this is the worker that serves the queue.
        body = None
        with contextlib.suppress(Exception):
            body = await request.json()
        return await self.respond(request, await self._in_thread(
            queue_api.append_request,
