...
Everything Music Assistant plays today comes from the Subsonic server the bridge
...
class MediaStreamRoute:
    """Serves `GET /ma_alexa_stream/<ref>.mp3` from any MA music provider."""
    ...
    async def handle(self, request: web.Request) -> web.StreamResponse:
        ...
        if not uri:
            raise web.HTTPNotFound(reason="not an Music Assistant stream reference")
