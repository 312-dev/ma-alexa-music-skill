...
group id Music Assistant discovered it under, so nothing needs translating.
...
`mediaReference.value.contentId` -- the `ext:<token>` Music Assistant itself published
...
class PushRouter:
    ...
    def resolve(self, event: Any, prefix: str) -> str | None:
        ...
        every device on the account, including ones Music Assistant does not expose and
