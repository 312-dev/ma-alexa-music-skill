# Music Assistant provider for Music Assistant

Exposes every Echo device and every Alexa speaker group as a Music Assistant
player. MA composes the queue; the bridge publishes it; Alexa plays it
...
docker run -d --name music-assistant \
  ...
  -v /opt/ma_alexa/ma_provider:/app/venv/lib/python3.14/site-packages/music_assistant/providers/ma_alexa:ro \
...
`Loaded player provider Music Assistant` appears whether or not the code is the code you
wrote. So the provider logs a digest of its own source at load:

```
ma_alexa provider build b033778ee608
```
...
services:
  music-assistant:
    ...
    volumes:
      ...
      - /opt/ma_alexa/ma_provider:/app/venv/lib/python3.14/site-packages/music_assistant/providers/ma_alexa:ro
```
...
```sh
docker exec music-assistant ls /app/venv/lib/python3.14/site-packages/music_assistant/providers/ma_alexa
```
...
`ask music assistant to play my mix on whole apartment` took four idle Echoes
to playing at once.

`play X on <alias>` is **not** used, spoken or otherwise. Typed, Alexa reads
`on <name>` as naming a speaker and goes looking for one called Music Assistant;
...
as an ordinary play does. Music Assistant has no reader for that buffer, so before this
...
By default Music Assistant learns what a speaker is doing by asking Amazon on a timer.
...
| Needs | companion API on `:5000` | the bridge |
