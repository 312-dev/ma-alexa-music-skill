...
logger = logging.getLogger("ma_alexa.setup")
...
def create_skill(*, alias: str, public_base: str, vendor: str = "") -> Outcome:
    ...
    word = (alias or current.get("alias") or "Music Assistant").strip()
...
def provision(*, alias: str, public_base: str, vendor: str = "") -> Outcome:
    """The provisioning steps in order, skipping whatever is already done.

    This is the whole of the one-button setup, minus the library upload. Each
    operation it calls is already idempotent on its own: create_skill returns
    early when a skill is recorded, and create_catalogs reuses catalogs by
    title. So this does not track its own "done" flags; it re-derives them from
    the same state the rail reads, and stops at the first hard failure with a
    message that names the step. A run that dies after the skill is created
    resumes at the catalogs on the next press, because the skill is now
    recorded and its own step no-ops.

    The upload is deliberately not here. It is minutes of work on a real
    library, so the caller hands it to Music Assistant's task controller once
    this returns ok, rather than blocking a config action for the length of a
    crawl. The upload re-binds the skill itself (cycle_after), so enablement is
    not a separate step either.
    """
    rows: list[dict] = []

    skill = create_skill(alias=alias, public_base=public_base, vendor=vendor)
    rows.append({"kind": "skill", "ok": skill.ok, "detail": skill.detail})
    if not skill.ok:
    ...
    if not all(catalog_ids().values()):
        ...
        rows.extend(catalogs.rows)
        if not catalogs.ok:
            return Outcome(False,
                           f"Setup stopped at the catalogs: {catalogs.detail}",
                           rows)
    else:
        rows.append({"kind": "catalogs", "ok": True,
                     "detail": "already created, reused"})

    return Outcome(True, "Skill and catalogs are ready. Uploading the library "
                         "next.", rows)
...
def run_upload(*, progress: Callable[[str, float | None], None] | None = None,
    ...
    try:
        ...
        # The catalog is built from whichever Music Assistant music providers the
        # operator selected. There is no implicit source: an empty selection is a
        ...
        if mass is not None and loop is not None:
            if not providers:
                return Outcome(False,
                               "Choose at least one music provider to catalog "
