<p align="center">
  <img src="brand/ma-icon.svg" width="96" height="96" alt="">
</p>
...
`play <thing> on music assistant` instead of `ask <skill> to play <thing>`, and you get
...
`music assistant` is just the default alias. You choose your own during setup, and the
...
- **Music Assistant.** This skill is a Music Assistant provider: it runs inside
  MA and is set up from MA's settings. There is no separate service any more.
- A Subsonic-compatible music server.
- A public HTTPS endpoint pointing at the port the skill listens on. Amazon calls
...
your own alias for `music assistant`.
...
| `play X from music assistant` | typed | provider dropped, plays from your default service |
...
data:
  ...
  media_content_id: "ask music assistant to play Gregory Alan Isakov on whole apartment"
```
