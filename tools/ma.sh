...
rsync -az -e "ssh -o BatchMode=yes" "$ROOT/tools/ma_cli.py" "$BOX:/opt/ampere/ma_cli.py"
...
docker cp /opt/ampere/ma_cli.py "app-$A:/tmp/ma_cli.py" >/dev/null
docker exec "app-$A" test -f /tmp/.ma-token 2>/dev/null || {
  docker cp /opt/ampere/.ma-token "app-$A:/tmp/.ma-token" >/dev/null
